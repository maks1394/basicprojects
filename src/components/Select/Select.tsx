...
type Props = DefaultDivType & {
    ...
    setValue: Dispatch<SetStateAction<string>> | Dispatch<SetStateAction<undefined>>
};
export const Select = ({children, setValue, value, ...restProps}: Props) => {
    ...
    if (Array.isArray(children)) {
        mappedOptions = children.map(el => {
            ...
            return <Option {...el.props} key={el.props.value} checked={checked} onClick={onClickOptionHandler}
                           setValue={setValue}/>
        })
    } else {
        if(children){
    ...
    const finalClassNameForSelect = s.select + (restProps.className ? ' ' + restProps.className : '')
    const finalClassNameForSelected = s.selected + (selectMode ? ' ' + s.selectMode : '')
    return (
        <div {...restProps} className={finalClassNameForSelect}>
            <div onClick={onClickSelectedHandler} className={finalClassNameForSelected}>{selected ? selected :
                <span className={s.span}>Select...</span>}{selectMode ? <IoIosArrowUp/> : <IoIosArrowDown/>}</div>
